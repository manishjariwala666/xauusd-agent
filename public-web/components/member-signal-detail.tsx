...
export function MemberSignalDetail({ publicId }: { publicId: string }) {
  ...
  return <>
    <section aria-labelledby="signal-levels"><h2 id="signal-levels">Protected signal levels</h2><dl><div><dt>Direction</dt><dd>{signal.direction || signal.signal_type || "—"}</dd></div><div><dt>Entry</dt><dd>{value(signal.entry_price)}</dd></div><div><dt>Entry range</dt><dd>{value(signal.entry_price_min)} – {value(signal.entry_price_max)}</dd></div><div><dt>Stop loss</dt><dd>{value(signal.stop_loss)}</dd></div><div><dt>Target 1</dt><dd>{value(signal.target_1)}</dd></div><div><dt>Target 2</dt><dd>{value(signal.target_2)}</dd></div><div><dt>Target 3</dt><dd>{value(signal.target_3)}</dd></div><div><dt>Target 4</dt><dd>{value(signal.target_4)}</dd></div><div><dt>Target 5</dt><dd>{value(signal.target_5)}</dd></div><div><dt>Target 6</dt><dd>{value(signal.target_6)}</dd></div></dl></section>
