...
export function MemberSignalPanel() {
  ...
  return <section className="member-panel"><div className="section-heading"><div><span className="eyebrow">VERIFIED MEMBER</span><h2>Protected Gold Signals</h2></div><button className="button secondary" type="button" onClick={logout}>Logout</button></div>
    ...
    {signals.length ? <div className="signal-grid">{signals.map((signal) => <article className="signal-card" key={signal.public_id}><div className="signal-card-head"><span>{signal.symbol || "XAUUSD"}</span><span>{signal.direction || signal.signal_type}</span></div><dl><div><dt>Entry</dt><dd>{price(signal.entry_price)}</dd></div><div><dt>Stop loss</dt><dd>{price(signal.stop_loss)}</dd></div><div><dt>Target 1</dt><dd>{price(signal.target_1)}</dd></div><div><dt>Target 2</dt><dd>{price(signal.target_2)}</dd></div><div><dt>Target 3</dt><dd>{price(signal.target_3)}</dd></div><div><dt>Target 4</dt><dd>{price(signal.target_4)}</dd></div><div><dt>Target 5</dt><dd>{price(signal.target_5)}</dd></div><div><dt>Target 6</dt><dd>{price(signal.target_6)}</dd></div></dl>{signal.analysis_summary && <p>{signal.analysis_summary}</p>}{signal.public_id && <Link href={`/signals/${encodeURIComponent(signal.public_id)}`}>Open member detail →</Link>}</article>)}</div> : !loadError ? <div className="empty-state"><h3>No published Gold Signal right now</h3><p>The protected feed shows only canonical published, non-deleted records.</p></div> : null}
  </section>;
}
