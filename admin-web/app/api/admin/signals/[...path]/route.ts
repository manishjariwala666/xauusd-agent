...
const allowedPath = /^\d+(\/(duplicate|publish|unpublish|transition|trash|restore))?$/;

async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  const path = (await context.params).path.join("/");
  if (!allowedPath.test(path)) return NextResponse.json({ message: "Not found." }, { status: 404 });
  if (request.method !== "GET" && !verifyCsrfToken(request.cookies.get(ADMIN_CSRF_COOKIE)?.value, request.headers.get("x-csrf-token"))) {
  ...
  try {
    ...
    const upstream = await fetch(`${config.backendBaseUrl}/admin/signals/${path}${request.nextUrl.search}`, {
  ...
  } catch { return NextResponse.json({ message: "Signals service is temporarily unavailable." }, { status: 503 }); }
}
