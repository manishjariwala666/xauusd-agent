...
export function GalleryManagerWorkspace() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        setGalleries(JSON.parse(saved) as SavedGallery[]);
      }
    } catch {
    ...
    void fetch(
      "/api/admin/media?page=1&page_size=100&state=active",
      {
        cache: "no-store",
  ...
  function saveGallery() {
    if (!title.trim()) {
      setMessage("Gallery title is required.");
      return;
    }
    ...
    setSelected([]);
    setColumns(3);
    setMessage("Gallery saved locally.");
  }

  function removeGallery(id: string) {
    const confirmed = window.confirm(
      "Delete this saved gallery?",
    );
      ...
      <div className="studio-gallery-builder">
        <div className="studio-gallery-fields">
          <label>
            ...
            <input
              ...
              placeholder="Homepage market gallery"
            />
          </label>
        ...
        {media.length ? (
          ...
          <div className="studio-media-empty">
            ...
            <span>Upload images in Media Library first.</span>
          </div>
        )}
      </div>
          ...
          galleries.map(gallery => (
            <article key={gallery.id}>
              <div>
                ...
                <small>
                  {gallery.mediaIds.length} images ·{" "}
