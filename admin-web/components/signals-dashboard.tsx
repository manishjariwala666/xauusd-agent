...
export function SignalsDashboard({
  ...
  return <section className="signals-admin-page">
    <header className="studio-header">
      <div>
        <span className="section-kicker">SIGNAL OPERATIONS</span>
        <h1>Signals</h1>
        <p>
          Review stored XAUUSD signals, levels and lifecycle status.
        </p>
      </div>
      {!readOnly && (
        <Link
    ...
    <section className="studio-panel signal-list-panel">
      ...
      {data.items.length ? <div className="signal-table-wrap"><table className="signal-table"><thead><tr><th>Signal</th><th>Entry</th><th>Stop loss</th><th>Targets</th><th>Status</th>
        <th>Signal date</th>
        <th>Signal time</th>
        <th>Last updated</th>
        <th>Actions</th></tr></thead><tbody>{data.items.map(item => <tr key={item.id}><td>
              {readOnly ? (
              ...
              <span
                className={`direction-chip ${item.signal_type.toLowerCase()}`}
              >
                {item.signal_type === "BUY" ? "↑ BUY" : "↓ SELL"}
              </span>
              <small>{item.timeframe}</small>
            </td><td>{price(item.price)}</td><td>{price(item.stop_loss)}</td><td>{[item.target_1,item.target_2,item.target_3,item.target_4].filter(Boolean).map(price).join(" · ") || "—"}</td><td><span className="status-badge draft"><i />{item.lifecycle_status.replaceAll("_", " ")}</span></td><td>{displayDate(item.signal_time)}</td>
            <td>{displayTime(item.signal_time)}</td>
            <td>
              <small>{displayDateTime(item.updated_at)}</small>
            </td><td>
              {readOnly ? (
                <span className="support-note">Read only</span>
              ) : (
                <SignalActions
                  id={item.id}
            ...
            </td></tr>)}</tbody></table></div> : <div className="state-panel"><strong>No signals match these filters.</strong><p>Create a synthetic draft or broaden the current filters.</p></div>}
