...
def save_result(*,actor_id:int,request_id:str,item_id:int|None=None,**v:Any)->dict[str,Any]:
    ...
    with session_scope() as s:
        ...
        if item_id is None:
            item_id=s.execute(text("""INSERT INTO verified_results(related_signal_id,symbol,direction,timeframe,entry_price,exit_price,stop_loss,targets,lifecycle_outcome,result_unit,result_value,result_points,opened_at,closed_at,evidence_type,evidence_media_id,evidence_notes,redaction_confirmed,public_summary,featured,created_by,updated_by) VALUES (:related_signal_id,:symbol,:direction,:timeframe,:entry_price,:exit_price,:stop_loss,CAST(:targets AS JSONB),:lifecycle_outcome,:result_unit,:points,:points,:opened_at,:closed_at,:evidence_type,:evidence_media_id,:evidence_notes,:redaction_confirmed,:summary,:featured,:actor,:actor) RETURNING id"""),p).scalar_one()
        else:
