...
TASKS: dict[str, dict[str, Any]] = {
    ...
    "run_market_data_agent": {
        ...
        "safe_payload": {"generate_signal": False, "give_signal": False, "recommend_trade": False, "place_trade": False, "send_telegram": False, "send_whatsapp": False, "publish_price": False, "modify_google_sheet": False},
    },
    "run_customer_support_agent": {
        "task_type": "CUSTOMER_SUPPORT", "title": "Prepare Customer Support Guidance", "agent_key": "customer_support_agent",
        ...
        "safe_payload": {"give_signal": False, "send_signal": False, "place_trade": False, "change_account": False, "reset_password": False, "process_payment": False, "process_refund": False, "delete_account": False, "send_email": False, "send_whatsapp": False, "send_telegram": False},
    },
    "run_marketing_strategy_agent": {
        "task_type": "MARKETING_STRATEGY", "title": "Prepare Marketing Strategy", "agent_key": "marketing_strategy_agent",
        ...
        "safe_payload": {"publish_social": False, "send_email": False, "send_telegram": False, "send_whatsapp": False, "create_backlink": False, "post_forum": False, "submit_directory": False, "start_campaign": False},
    },
    "run_social_media_agent": {
        "task_type": "SOCIAL_MEDIA", "title": "Prepare Social Media Drafts", "agent_key": "social_media_agent",
...
def _load_completed_step_output(run_id: int, agent_key: str) -> str | None:
    try:
        ...
        with session_scope() as session:
            row = session.execute(
                text("""SELECT output_summary FROM public.master_ai_execution_steps WHERE run_id = :run_id AND agent_key = :agent_key AND status = 'COMPLETED' ORDER BY finished_at DESC NULLS LAST, id DESC LIMIT 1"""),
...
UNSAFE_ERROR_MARKERS = (
    ...
    "postgres://", "postgresql://", "/app/", "/home/", "/users/", "/private/", "/var/", "c:\\",
)


def execute_master_ai_action(
    action: str, *, source: str = "MASTER_AI", runner: Callable[..., Any] = create_and_start_master_task,
    input_payload: dict[str, Any] | None = None, status_loader: Callable[..., list[dict[str, Any]]] | None = None,
    retry_action: str | None = None, supabase: Any | None = None,
) -> MasterAIToolResult:
